...
def extract_only_ref_variant_fasta():
    ...
    for i in vcf_filenames:
        ...
        for lines in ffp:
            #next(ffp)
            lines = lines.strip()
            grep_position = "zcat %s | grep -w \'%s\' | awk -F\'\\t\' \'{print $5}\'" % (core_vcf_file, lines)
            #print grep_position
...
# def extract_only_ref_variant_fasta_jobs(i):
#     f = Fasta(args.reference)
#     if len(f.keys()) == 1:
#         ref_id = str(f.keys())
#
#     ffp = open("%s/Only_ref_variant_positions_for_closely" % args.filter2_only_snp_vcf_dir, 'r+')
#     core_vcf_file = i.replace('_filter2_final.vcf_no_proximate_snp.vcf', '_filter2_final.vcf_core.vcf.gz')
#     print "Processing %s" % core_vcf_file
#     #fasta_string = ">%s\n" % core_vcf_file.replace('_filter2_final.vcf_core.vcf.gz', '')
#     fasta_string = ""
#     for lines in ffp:
#         next(ffp)
...
#         fp.close()
#         ffp.close()
#     #sequence_lgth_cmd = "bioawk -c fastx \'{ print $name, length($seq) }\' < %s/%s_variants.fa" % (args.filter2_only_snp_vcf_dir, os.path.basename(core_vcf_file.replace('_filter2_final.vcf_core.vcf.gz', '')))
#     #os.system(sequence_lgth_cmd)
